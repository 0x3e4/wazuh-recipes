#!/usr/bin/env python3
"""Wazuh "PowerShell / LOLBins" weekly digest.
...
Watched Wazuh rules (suspicious PowerShell / living-off-the-land binaries):
  91809  PowerShell using Base64 decoding
  91822  PowerShell "Invoke-command" used to execute a sub-script
  91846  PowerShell .NET compression (possible data exfiltration)
  92041  Value added to a registry key has a Base64-like pattern
  92055  FodHelper.exe may have been used to bypass UAC
  92071  PowerShell created by WMI executed a Base64-encoded command
  92206  DLL created by the print spooler (possible PrintNightmare)
  92212  Suspicious file-compression activity by PowerShell
  92910  Explorer accessed by PowerShell (possible injection)
  92920  RDP utility accessed by PowerShell (possible injection)

Indexer filter:  rule.id:(91809 OR 91822 OR 91846 OR 92041 OR 92055 OR 92071 OR 92206 OR 92212 OR 92910 OR 92920)

Config comes from the environment / an env file (see powershell-lolbins-digest.env.example).
...
TOPIC        = "PowerShell / LOLBins"
RULE_IDS     = ["91809", "91822", "91846", "92041", "92055",
                "92071", "92206", "92212", "92910", "92920"]
...
RULE_FILTER  = ("rule.id:(91809 OR 91822 OR 91846 OR 92041 OR 92055 OR 92071 "
                "OR 92206 OR 92212 OR 92910 OR 92920)")
...
# Rule level -> header accent colour (these rules are mostly L10..L14).
...
def collect_summary(client, index_pattern, range_days):
    ...
    Returns a dict with: total, by_rule (description -> count), by_agent
    (host -> count), and by_user (eventdata user -> count).
    """
    body = {
        "size": 0,
        ...
        "aggs": {
            ...
            "by_user":  {"terms": {"field": "data.win.eventdata.user", "size": _TOP_N}},
        },
    }
    ...
    return {
        ...
        "by_user":  _buckets("by_user"),
    }

def _collapse_bs(s):
    """Sysmon eventdata sometimes doubles backslashes (C:\\\\x); show single."""
...
def collect_recent(client, index_pattern, range_days, size=25):
    ...
    body = {
        "size": size,
        "_source": ["timestamp", "agent.name", "rule.id", "rule.description", "rule.level",
    ...
    for h in resp.get("hits", {}).get("hits", []):
        ...
        # the most informative of commandLine / image for this event
        detail = _collapse_bs(ed.get("commandLine") or ed.get("image") or "")
        rows.append({
            "timestamp":   s.get("timestamp", ""),
            "agent":       a.get("name", ""),
            "user":        _collapse_bs(ed.get("user", "")),
            "detail":      detail,
            "rule_id":     r.get("id", ""),
...
def build_text(summary, recent, range_days, now):
    ...
    L.append("")
    L.append(f"## Top detections (rule.description) - {len(summary['by_rule'])} kind(s)")
    if summary["by_rule"]:
        ...
        L.append("  (none)")
    L.append("")
    L.append(f"## Top agents (agent.name) - top {_TOP_N}")
    if summary["by_agent"]:
        ...
        L.append("  (none)")
    L.append("")
    L.append(f"## Top users (data.win.eventdata.user) - top {_TOP_N}")
    if summary["by_user"]:
        for u, n in summary["by_user"]:
            L.append(f"  {n:>6}  {u}")
    else:
        L.append("  (none)")
    L.append("")
    ...
    if recent:
        for r in recent:
            L.append(f"  {_fmt_ts(r['timestamp'])}  L{r['rule_level']}  {r['agent']}"
                     f"  [{r['user']}]  {r['rule_desc']}")
            if r["detail"]:
                L.append(f"      {r['detail'][:200]}")
    else:
        L.append("  (none)")
    L.append("")
...
def build_html(summary, recent, range_days, now):
    ...
    intro = (
        ...
        f'<strong>{summary["total"]}</strong> suspicious PowerShell / LOLBin detection(s) over the '
        f'last <strong>{int(range_days)}</strong> day(s) across '
        f'<strong>{len(summary["by_agent"])}</strong> agent(s); '
        f'<strong>{len(summary["by_rule"])}</strong> distinct detection(s).<br>'
    ...
    user_rows  = [(_esc(u),    f'<strong>{n}</strong>') for u,    n in summary["by_user"]]
    ...
    rec_tbl = _kv_table_html(
        ["Time (UTC offset)", "Agent", "User", "Level", "Detection", "Command / image"], rec_rows)

    body = (
        _section_bar_html("Top detections") + _kv_table_html(["Detection (rule.description)", "Count"], rule_rows)
        + _section_bar_html("Top agents") + _kv_table_html(["Agent (agent.name)", "Count"], agent_rows)
        + _section_bar_html("Top users") + _kv_table_html(["User (data.win.eventdata.user)", "Count"], user_rows)
...
def run(args):
    ...
    print(f"[digest] topic={TOPIC!r} indexer={args.index_url} index={args.index_pattern} "
          ...
          f"total={summary['total']} detections={len(summary['by_rule'])} "
          f"agents={len(summary['by_agent'])} users={len(summary['by_user'])}")
