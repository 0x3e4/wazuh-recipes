...
| [clamav](clamav/) | Collect ClamAV malware detections, scan errors and signature-update health from Ubuntu/Debian agents into Wazuh — a log-file decoder plus rules (virus found, outbreak correlation, update failures) and a dashboard. |
...
| [windows-defender](windows-defender/) | Ingest Microsoft Defender Antivirus detections, remediation, tamper (real-time-protection/scanning disabled, exclusions) and health into Wazuh — enables the Defender Operational eventchannel on the windows group, adds detection/tamper rules, and a dashboard. |
